...
(define (highest-type tags)
  (foldl (lambda (t1 t2)
           (cond
             [(reachable? t1 t2) t2]
             [(reachable? t2 t1) t1]
             [else (error "max-type: incomparable tags" t1 t2)]))
         (first tags)
         (rest tags)))

(define (reachable? from to)
  (let loop ((t from))
    (cond
      [(eq? t to)               #t]
      [(not (type-raised-to t)) #f]
      [else (loop (type-raised-to t))])))

(define (type-raised-to tag)
  (get 'type-raised-to tag))

;; -- apply generic --

(define (apply-generic op . args)
  (define tags   (map type-tag args))
  (define values (map contents args))
  (define proc   (get op tags))
  (display op)
  (newline)
  (display args)
  (newline)
  ...
  (cond (proc (apply proc values))
        ((> (length args) 1)
         (let ([target (highest-type tags)])
           (define (raise-to v)
             (let loop ([x v])
               (if (eq? (type-tag x) target)
                   x
                   (loop (raise x)))))
           (apply apply-generic op (map raise-to args))))
...
(define (install-scheme-number-package)
  ...
  (put 'type-raised-to 'scheme-number 'rational)
  'done)
...
(define (install-rational-package)
  ...
  (put 'type-raised-to 'rational 'complex)
  ;; coercions
...
(define (install-complex-number-package)
  ...
  (define (rational->complex x)
    (make-complex-from-real-imag (attach-tag 'rational x) 0))
